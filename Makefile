...
install:
	@cd ../installer; \
	helm upgrade -i secrets-store-reader charts/secrets-store-reader --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall secrets-store-reader --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/secrets-store-reader run \
		--v=3 \
		--secure-port=8443 \
